...
df_nibe_casts <- bind_rows(
  ...
  filter(lead(abilityGameID) %in% c(71843,71844) | lag(abilityGameID) %in% c(71843,71844) | abilityGameID %in% c(71843,71844)) %>%
...
```{r}
df_nibe_casts %>% filter(type=="summon") %>% write.csv(paste0("./clean_data_Nibelung_PTR_",format(Sys.time(), "%Y_%m_%d_h%H_m%M"),".csv"), row.names = F)
